...
const Room = () => {
	...
	function toggleMic(){
		const audioTrack = currentStreamRef.current?.getTracks().find(track => track.kind === 'audio');
	...
	function toggleCamera(){
		const videoTrack = currentStreamRef.current?.getTracks().find(track => track.kind === 'video');
		...
		<div className="h-screen w-full bg-zinc-900 grid grid-cols-3 p-6 gap-6">
			<div className="fixed bottom-5 right-5 flex flex-col gap-y-3">
				<div className="flex flex-col gap-y-2">
					<button onClick={toggleMic} className="bg-amber-500 rounded-sm text-sm px-4 py-1">{toggleMicText}</button>
					<button onClick={toggleCamera} className="bg-amber-500 rounded-sm text-sm px-4 py-1">{toggleCameraText}</button>
					{sharingScreen ? (
						<button
