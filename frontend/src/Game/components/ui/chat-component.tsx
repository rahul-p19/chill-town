...
const ChatUI: React.FC<{
  ...
  // Handle input change, ensuring spaces are preserved
  ...
  return (
    <div className="absolute bottom-15 left-2 z-50 font-pixelify">
      <div className="w-80 rounded p-2 text-white bg-black/60">
        {/* Messages display area */}
        <div className="h-32 overflow-y-auto mb-2 p-1">
...
export class ChatComponent {
  ...
  addMessage(sender: string, text: string) {
    // Ensure text is treated as a string with spaces preserved
    this.messages.push({ sender, text: String(text) });
    // Limit to most recent 5 messages
    if (this.messages.length > 5) {
      this.messages.shift();
    }
